...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, vol_sizes=None):
        """
        hidden_states: (B, L, D)
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            if self.sp_bimamba:
                ...
                xz_s = xz.chunk(t, dim=-1)  # (B, D, L) = (B, D, T * L') -> T (B, D, L')
                xz_s = torch.stack(xz_s, dim=-1)  # (B, D, L', T)
                xz_s = xz_s.flatten(-2)  # (B, D, L' * T)
                ...
                # (B, D, L' * T) -> (B, D, L', T) -> (B, D, T, L') -> (B, D, T * L')
                out_s = out_s.reshape(batch, self.d_inner, seqlen // t, t).permute(0, 1, 3, 2).flatten(-2)
