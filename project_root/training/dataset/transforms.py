...
class RandomHorizontalFlip:
    def __init__(self, consistent_transform, p=0.5):
        ...
        # only support consistent transform for now and raise error if not
        if not consistent_transform:
            raise NotImplementedError

    def __call__(self, datapoint, **kwargs):
        assert self.consistent_transform
        if self.consistent_transform:
            if random.random() < self.p:
                # if the image is flipped, we need to flip the expression
                for i in range(len(datapoint.expression)):
                    datapoint.expression[i] = datapoint.expression[i].replace('left', '_').replace('right', 'left').replace('_', 'right')
...
class RandomAffine:
    def __init__(
        ...
        self.allow_first_zero_mask = allow_first_zero_mask
        ...
        for img_idx, img in enumerate(datapoint.frames):
            ...
            for i in range(len(img.objects)):
                if this_masks[i] is None:
                    ...
                    if img_idx == 0 and transformed_mask.max() == 0 and not self.allow_first_zero_mask:
