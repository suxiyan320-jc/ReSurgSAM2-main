...
def load_checkpoint_and_apply_kernels(
    ...
    Returns: Model with the matchin pre-trained weights loaded.
    """
    # 循环扫描ckpt_path，直到文件存在
    while not g_pathmgr.exists(checkpoint_path):
        logging.info(f"Waiting for checkpoint file {checkpoint_path} to exist.")
        time.sleep(60)
...
def load_state_dict_into_model(
    ...
    # # 删除state_dict中的temporal_pos_embed预训练中使用
    # state_dict.pop('cross_modal_fusion.temporal_pos_embed', None)
    # 遍历，根据名字删除，删除含有dwconv
    # for key in list(state_dict.keys()):
    #     if 'dwconv' in key and 'cross_modal_fusion' in key:
